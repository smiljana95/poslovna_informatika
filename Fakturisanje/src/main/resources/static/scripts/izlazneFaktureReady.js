$(document).ready(function () {
	...
	$.ajax({
        ...
        success: function (data) {
        	if(data.role === "REGISTERED") {
				...
				$.ajax({
						async: false,
						url: "http://localhost:1234/faktura/getIzlazneFakture/"+idPP,
				        type: "GET",
				        dataType: "json",
				        success: function (data) {
				        	...
				        	for(i=0; i<data.length; i++) {
				        		...
				        		str +="<button onclick=\"eksportujFakturu("+data[i].id+")\"  class=\"btn btn-default\">Eksportuj fakturu</button>&nbsp";
				        		str +="<button onclick=\"otpremiFakturu("+data[i].id+")\"  class=\"btn btn-default\">Otpremi fakturu</button>";
				        		//str +="&nbsp&nbsp&nbsp<button onclick=\"obrisiIzlaznuFakturu("+data[i].id+")\" class=\"btn btn-danger\">Obrisi narudzbenicu</button>";
				        		str +="</div><br/><br/>";
				        	}
				        	div.append(str);
				        },
				        error: function (jqxhr, textStatus, errorThrown) {
				        	toastr['warning'] ('Poslovni partner nema cenovnikk');
	...
	$("#inputDateFaktura").on('input propertychange', function() {
		  prikaziNasCenovnik();
		});
})